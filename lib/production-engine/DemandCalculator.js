...
export class DemandCalculator {
     ...
     */
    static processIngredientOrExplode({ ing, parentScale, allRecipes, contextStr, visitedIds, originId, onLeafFound, inheritedTaskType }) {
        ...
        if (type === 'food') {
            ...
            if (matchingRecipe && !visitedIds.has(matchingRecipe.id)) {
                ...
                if (defYield > 0) {
                    ...
                    // Pass down this ingredient's task type to sub-components
                    const currentTaskType = ing.task_type !== undefined ? ing.task_type : inheritedTaskType;
                    ...
                    this.recursiveExplode({
                        ...
                        visitedIds: new Set(visitedIds),
                        onLeafFound: (leafIng, sQty, ctx, orig) => {
                            if (leafIng.task_type === undefined && currentTaskType !== undefined) {
                                leafIng = { ...leafIng, task_type: currentTaskType };
                            }
                            onLeafFound(leafIng, sQty, ctx, orig);
                        }
                    });
            ...
            // IF visitedIds.has(matchingRecipe.id) is true -> Loop Detectado:
            // Isso significa que estamos dentro da receita "Arroz Branco", 
            // processando o ingrediente "Arroz Branco".
            // Nesse caso, o ingrediente não é um link para a receita externa, 
            // ele é a "folha" real. O fluxo avança naturalmente para a linha abaixo sem retornar.
        }
        ...
        let resolvedTaskType = ing.task_type !== undefined ? ing.task_type : inheritedTaskType;
