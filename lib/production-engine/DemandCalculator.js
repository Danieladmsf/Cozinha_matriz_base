...
export class DemandCalculator {
    ...
    static explodeOrders(orders, allRecipes, categoryMap) {
        ...
        orders.forEach(order => {
            ...
            order.items.forEach(orderItem => {
                ...
                if (recipe.name.toLowerCase().includes('arroz')) {
                    console.log('🔍 [DEMAND] Found Order Item:', recipe.name, 'QTY:', orderItem.quantity, 'UNIT:', orderItem.unit_type);
                }
                ...
                if (baseRecipe.name.toLowerCase().includes('arroz')) {
                    console.log('🔍 [DEMAND] Exploding baseRecipe:', baseRecipe.name, 'Scale:', scaleFactor, 'Yield:', recipeYieldWeight, 'Ordered:', orderedQty);
                }
    ...
    static processIngredientOrExplode({ ing, parentScale, allRecipes, contextStr, visitedIds, originId, onLeafFound }) {
        ...
        if (ingName.toLowerCase().includes('arroz')) {
            console.log('🔍 [EXPLODE] processIngredient:', ingName, 'raw qty:', ing.weight_raw, 'pre_cooking:', ing.weight_pre_cooking, 'quantity:', ing.quantity);
        }
        ...
        if (isPkg) {
            ...
            // Fallback: se o peso bruto for 0, tenta usar o peso de rendimento (o usuário preencheu apenas o "Peso Rend.")
            if (qtyRaw <= 0) {
                qtyRaw = RecipeEngine.getFinalWeight(ing);
            }
            type = 'food';
        }
        ...
        if (qtyRaw <= 0) {
            if (ingName.toLowerCase().includes('arroz')) {
                console.log('🔍 [EXPLODE] Discarded strictly because qtyRaw <= 0:', ingName);
            }
            return;
        }
        ...
        if (type === 'food') {
            ...
            if (matchingRecipe && !visitedIds.has(matchingRecipe.id)) {
                if (ingName.toLowerCase().includes('arroz')) {
                     console.log('🔍 [EXPLODE] Implicit link found for:', ingName, 'defYield:', this.getRecipeYieldWeight(matchingRecipe, allRecipes));
                }
            ...
            } else if (matchingRecipe && visitedIds.has(matchingRecipe.id)) {
                 if (ingName.toLowerCase().includes('arroz')) {
                     console.log('🔍 [EXPLODE] Implicit link loop detected. Falling through to leaf:', ingName);
                 }
            }
        }
        ...
        if ((ingName || '').toLowerCase().includes('berinjela') || (ingName || '').toLowerCase().includes('arroz')) {
            console.log('🔍 [LEAF DEBUG]', {
                ingName,
                qtyRaw,
                parentScale,
                scaledQty,
