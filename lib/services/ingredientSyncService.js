...
export async function syncIngredientAcrossRecipes(ingredientId, updatedFields, oldName = null) {
    ...
    try {
        ...
        log(`[DEBUG-TACO] [SyncService] Iniciado para Insumo Mestre ID: '${ingredientId}' | NomeAntigo: '${oldName}'`);
        log(`[DEBUG-TACO] [SyncService] Dados Recebidos (fieldsToSync): ${JSON.stringify(updatedFields, null, 2)}`);
        ...
        for (const recipe of allRecipes) {
            ...
            const updatedPreparations = recipe.preparations.map((prep, pIndex) => {
                ...
                const updatedIngredients = prep.ingredients.map((ing, iIndex) => {
                    const masterIdStr = String(ingredientId).trim();
                    const ingIdInRecipe = String(ing.id || ing.ingredient_id || "").trim();

                    // 1. Match Direto por ID (Padrão Ouro)
                    const matchById = masterIdStr !== "" && (
                        ingIdInRecipe === masterIdStr ||
                        String(ing.source_ingredient_id || "").trim() === masterIdStr
                    );

                    // 2. Match por Nome (Recuperação de Órfãos/Legados)
                    // Normalização Profunda: remove acentos, espaços extras e ignora maiúsculas
                    ...
                    const normalizedMasterName = normalize(updatedFields.name || oldName);
                    const normalizedIngName = normalize(ing.name);
                    
                    const matchByName = normalizedMasterName !== "" && normalizedMasterName === normalizedIngName;

                    if (matchById || matchByName) {
                         let repairReason = "";
                         if (matchById) repairReason = "ID_MATCH";
                         else {
                             repairReason = "NAME_REPAIR (ORPHAN ADOPTED)";
                             console.warn(`[DEBUG-TACO] [REPAIR] 💊 Órfão Encontrado! Vinculando '${ing.name}' ao ID mestre '${masterIdStr}' na receita '${recipe.name}'`);
                         }

                         log(`[SyncService] 🎯 MATCH (${repairReason}) em '${recipe.name}' no ingrediente '${ing.name}'`);

                         // Injetar dados TACO, Preço e Metadados Totais
                         ...
                         // Garantir consistência nas Variações TACO
                         let chosenTacoUpdate = {};
                         if (updatedFields.taco_variations && Array.isArray(updatedFields.taco_variations)) {
                            const currChosen = ing.chosen_taco_id || ing.taco_id;
                            const stillExists = updatedFields.taco_variations.some(v => v.taco_id === currChosen);
                            
                            if (!stillExists || !currChosen) {
                                const newBase = updatedFields.taco_variations.find(v => v.is_base) || updatedFields.taco_variations[0];
                                if (newBase) {
                                     chosenTacoUpdate.chosen_taco_id = newBase.taco_id;
                                     chosenTacoUpdate.chosen_variation_name = newBase.variation_name;
                                }
                            }
                         }

                         recipeModified = true;
                         // RETORNA O INGREDIENTE REPARADO: Agora com o ID mestre injetado na receita!
                         return {
                            ...ing,
                            ...updatedFields,
                            id: masterIdStr,           // CURA O ID NA RAIZ
                            ingredient_id: masterIdStr, // VINCULO ABSOLUTO
                            ...priceUpdate,
                            ...chosenTacoUpdate
                         };
                    }
