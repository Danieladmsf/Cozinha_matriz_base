...
export class RecipeEngine {
  ...
  static getInitialWeight(ingredient, processes = []) {
    ...
    if (processes.length > 0) {
      if (processes.includes('cleaning')) {
        if (this._hasExplicitValue(ingredient.weight_raw)) {
            const parsed = this.parseValue(ingredient.weight_raw);
            if (parsed > 0) return parsed;
        }
        if (this._hasExplicitValue(ingredient.weight_thawed)) {
            const parsed = this.parseValue(ingredient.weight_thawed);
            if (parsed > 0) return parsed;
        }
      }
      ...
      if (processes.includes('defrosting')) {
        if (this._hasExplicitValue(ingredient.weight_frozen)) {
            const parsed = this.parseValue(ingredient.weight_frozen);
            if (parsed > 0) return parsed;
        }
      }
    }
    ...
    for (const field of WEIGHT_FIELDS.INITIAL_PRIORITY) {
      if (this._hasExplicitValue(ingredient[field])) {
        ...
        if (parsed > 0) return parsed; // Pega o primeiro preenchido > 0
      }
    }
  ...
  static calculateIngredientCost(ingredient, processes = []) {
    ...
    if (processes.includes('packaging')) {
      return price;
    }
  ...
  static getContainerType(preparations) {
    ...
    return assemblyPrep?.assembly_config?.container_type || 'unidade';
  }
  ...
  static getEmptyRecipeMetrics() {
    return {
      ...
      weight_field_name: 'Peso da Unidade',
