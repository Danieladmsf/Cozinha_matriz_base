...
export default function IngredientsTable({ ingredients, onDelete, updateIngredientPrice, updateIngredient, itemType = 'ingrediente' }) {
  ...
  // Chave para persistência baseada no tipo (ingrediente ou embalagem)
  const storageKey = `sort_config_${itemType}`;

  // Estado de ordenação - Inicializa do localStorage
  ...
  // Salvar ordenação no localStorage sempre que mudar
  useEffect(() => {
    if (typeof window !== 'undefined') {
    ...
    if (sortConfig.key) {
      filtered.sort((a, b) => {
        ...
        if (sortConfig.key === 'displayPrice') {
          ...
          aValue = aValue ? 1 : 0;
          bValue = bValue ? 1 : 0;
        } else if (sortConfig.key === 'taco_count') {
          aValue = a.taco_variations?.length || 0;
          bValue = b.taco_variations?.length || 0;
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-gradient-to-r from-slate-50 to-slate-100 border-b border-slate-300">
                  ...
                  <th
                    className="text-left px-3 py-2 text-xs font-semibold text-slate-600 cursor-pointer hover:bg-slate-100 transition-colors whitespace-nowrap"
                    onClick={() => handleSort('taco_count')}
                  >
                    <div className="flex items-center gap-1">
                      <Leaf className="w-3 h-3 text-emerald-500" />
                      <span>Vínculo TACO</span>
                      <SortIcon columnKey="taco_count" />
                    </div>
                  </th>
                    ...
                    >
                      ...
                      <td className="px-3 py-2 font-mono text-xs whitespace-nowrap">
                        {ingredient.taco_variations && ingredient.taco_variations.length > 0 ? (
                          <div 
                            className="flex items-center gap-1.5 bg-emerald-50 text-emerald-700 px-2 py-0.5 rounded-full border border-emerald-100 w-fit cursor-pointer hover:bg-emerald-100 hover:border-emerald-200 transition-all shadow-sm active:scale-95"
                            onClick={(e) => {
                              e.stopPropagation();
                              router.push(`/ingredientes/editor?id=${ingredient.id}&tab=taco`);
                            }}
                            title="Ver variações vinculadas"
                          >
                            <Leaf className="w-3 h-3" />
                            <span className="font-semibold">{ingredient.taco_variations.length}</span>
                            <span className="text-[10px] hidden lg:inline">Variações</span>
                          </div>
                        ) : (
                          <div 
                            className="flex items-center gap-1.5 text-slate-400 px-2 py-0.5 cursor-pointer hover:text-emerald-600 transition-colors"
                            onClick={(e) => {
                              e.stopPropagation();
                              router.push(`/ingredientes/editor?id=${ingredient.id}&tab=taco`);
                            }}
                            title="Vincular a tabela TACO"
                          >
